...
``` r
city_results_arrange|>ggplot(aes(x=reorder(city_state, estimate), y = estimate, color=city_state)) +
  geom_point() +
...
![](p8105_hw6_xw3038_files/figure-gfm/unnamed-chunk-9-1.png)<!-- -->
...
``` r
birthweight=read_csv(file = "./data/birthweight.csv", na = c(".", "NA", "Unknown")) |>
  janitor::clean_names()
```
...
cv_df=cv_df |> 
  mutate(
    ...
    original_mod = map(train, \(df) lm(bwt ~babysex + blength + bhead + momage + mheight + ppbmi +
                              smoken + wtgain + menarche+ fincome, data = df)),
  ...
  mutate(
    ...
    rmse_model3 = map2_dbl(model3_mod, test, \(mod, df) rmse(model = mod, data = df))
  )
```
...
![](p8105_hw6_xw3038_files/figure-gfm/unnamed-chunk-18-1.png)<!-- -->
